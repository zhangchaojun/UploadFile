package com.example.uploadfile.client;
...
public class FileUploadClientHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelActive(ChannelHandlerContext ctx) {
        ...
        try {
            randomAccessFile = new RandomAccessFile(fileUploadFile.getFile(), "rw");
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof Integer) {
            ...
            if (start != -1) {
                ...
                Log.e(TAG, "块儿长度：" + (randomAccessFile.length() / 10));
                Log.e(TAG, "长度：" + (randomAccessFile.length() - start));
                ...
                if ((byteRead = randomAccessFile.read(bytes)) != -1 && (randomAccessFile.length() - start) > 0) {
                    Log.e(TAG, "byte 长度：" + bytes.length);
                    fileUploadFile.setEndPos(byteRead);
                    fileUploadFile.setBytes(bytes);
                    try {
                        ctx.writeAndFlush(fileUploadFile);
